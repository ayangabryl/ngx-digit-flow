...
@Component({
  ...
  template: `
    <div class="demos-page">
      ...
      <div class="bento">
        ...
        <div class="card card--trend" data-demo-card="stock">
          ...
          <div class="trend-price" [class.trend-up]="trendUp()" [class.trend-down]="!trendUp()">
            <ngx-digit-flow [value]="stockPrice()" [format]="stockFmt" [duration]="600" />
          </div>
        ...
        <!-- Countdown -->
        <div class="card card--countdown" data-demo-card="countdown">
        ...
        <!-- Score -->
        <div class="card card--score" data-demo-card="score">
          <div class="score-head">
            <span class="card-label">Score</span>
            <span class="score-best"
              >Best&nbsp;<strong>{{ scoreBest() }}</strong></span
            >
          </div>
          <div class="score-stage">
            <div
              class="score-number"
              [class.trend-up]="scoreTrend() === 'up'"
              [class.trend-down]="scoreTrend() === 'down'"
            >
            ...
            @if (scoreDelta() !== 0) {
              <span
                class="score-delta"
                [class.pos]="scoreDelta() > 0"
                [class.neg]="scoreDelta() < 0"
              >
        ...
        <!-- XP / Level -->
        <div class="card card--xp" data-demo-card="xp">
          <div class="xp-head">
          ...
          <div class="xp-bar-wrap">
            ...
            <span class="xp-pct"
              ><ngx-digit-flow [value]="xpPctDisplay()" [duration]="500" />%</span
            >
          </div>
          <span class="xp-to-next">
            <ngx-digit-flow
              class="xp-to-next-num"
              [value]="xpToNext()"
              [format]="xpFmt"
              [duration]="500"
            />&nbsp;XP to Lv&nbsp;{{ xpLevel() + 1 }}
        ...
        <!-- Progress -->
        <div class="card card--progress" data-demo-card="progress">
          <div class="progress-head">
            <span class="card-label">Progress</span>
            <button
              class="progress-play-btn"
              (click)="progressPaused.update((v) => !v)"
              [attr.aria-label]="progressPaused() ? 'Play' : 'Pause'"
            >
              @if (progressPaused()) {
          ...
          <div class="progress-style-tabs">
            <button
              [class.active]="progressStyle() === 'linear'"
              (click)="progressStyle.set('linear')"
            >
              Linear
            </button>
            <button
              [class.active]="progressStyle() === 'steps'"
              (click)="progressStyle.set('steps')"
            >
              Steps
            </button>
            <button [class.active]="progressStyle() === 'ring'" (click)="progressStyle.set('ring')">
              Ring
            </button>
          </div>
        </div>
        ...
        <!-- Slider — 2 cols × 1 row -->
        <div class="card card--slider" data-demo-card="slider">
          <span class="card-label">Slider</span>
          <div class="card-center slider-value">
            <ngx-digit-flow [value]="sliderVal()" [format]="sliderFmt" [duration]="300" />
          </div>
        ...
        <!-- Pricing tiers -->
        <div class="card card--pricing" data-demo-card="pricing">
          ...
          <div class="pricing-body">
            ...
            <div class="pricing-amount">
              <ngx-digit-flow [value]="pricingVal()" [format]="pricingFmt" [duration]="700" />
            </div>
        ...
        <!-- Temperature toggle -->
        <div class="card card--temp" data-demo-card="temperature">
          ...
          <div class="temp-btns">
            <button class="temp-btn" [class.active]="tempCelsius()" (click)="tempCelsius.set(true)">
              °C
            </button>
            <button
              class="temp-btn"
              [class.active]="!tempCelsius()"
              (click)="tempCelsius.set(false)"
            >
              °F
            </button>
          </div>
        ...
        <!-- Transfer -->
        <div class="card card--transfer" data-demo-card="transfer">
          ...
          <div class="transfer-row">
            ...
            <div class="transfer-side transfer-side--recv">
              ...
              <ngx-digit-flow
                class="transfer-val transfer-val--recv"
                [value]="transferReceive()"
                [format]="transferFmt()"
                [duration]="400"
              />
            </div>
          ...
          <input
            ...
            step="10"
            [value]="transferAmount()"
            (input)="onTransferChange($event)"
          />
          <div class="transfer-rate">
            <span class="transfer-rate-text">
              1 USD =&nbsp;<ngx-digit-flow
                class="transfer-rate-num"
                [value]="transferRate()"
                [format]="transferRateFmt()"
                [duration]="700"
              />&nbsp;{{ transferPair().code }}
            </span>
        ...
        <!-- Social counter -->
        <div class="card card--social" data-demo-card="social">
          <span class="card-label">Social</span>
          <div class="social-post">
            <img
              class="social-avatar"
              src="https://profilio.ai/brand/profilio-logo-32.png"
              alt="avatar"
            />
            <p class="social-body">
              Numbers that feel alive. ngx-digit-flow makes every counter a micro-experience in
              Angular.
            </p>
          </div>
          <div class="social-stats">
            ...
            <button
              class="social-stat social-stat--heart"
              [class.liked]="socialLiked()"
              (click)="toggleLike()"
            >
              <svg
                width="15"
                height="15"
                viewBox="0 0 24 24"
                [attr.fill]="socialLiked() ? 'currentColor' : 'none'"
                stroke="currentColor"
                stroke-width="1.8"
                stroke-linecap="round"
                stroke-linejoin="round"
        ...
        <!-- Cart -->
        <div class="card card--cart" data-demo-card="cart">
            ...
            <div class="cart-item">
              ...
              <div class="cart-right">
                <div class="cart-qty-ctrl">
                  <button
                    class="cart-qty-btn"
                    (click)="cartQty1.update((v) => (v > 0 ? v - 1 : 0))"
                  >
                    −
                  </button>
            ...
            <div class="cart-item">
              ...
              <div class="cart-right">
                <div class="cart-qty-ctrl">
                  <button
                    class="cart-qty-btn"
                    (click)="cartQty2.update((v) => (v > 0 ? v - 1 : 0))"
                  >
                    −
                  </button>
  ...
  styles: [
    `
      ...
      .score-number.trend-up {
        color: oklch(42% 0.18 145);
      }
      .score-number.trend-down {
        color: oklch(44% 0.18 25);
      }
      ...
      .score-btn--inc {
        border: 1px solid oklch(52% 0.2 145);
        background: oklch(52% 0.2 145);
        color: #fff;
      }
      .score-btn--inc:hover {
        background: oklch(44% 0.2 145);
        border-color: oklch(44% 0.2 145);
      }
      ...
      .xp-levelup-overlay {
        ...
        inset: 0;
        border-radius: 24px;
        background: linear-gradient(145deg, oklch(36% 0.24 280), oklch(44% 0.2 310));
        display: flex;
        align-items: center;
        justify-content: center;
        z-index: 10;
      ...
      @keyframes xpSparkBurst {
        ...
        30% {
          opacity: 1;
          transform: scale(1.6) translate(calc(var(--xp-tx) * 0.4), calc(var(--xp-ty) * 0.4));
        }
        to {
          opacity: 0;
          transform: scale(0.4) translate(var(--xp-tx), var(--xp-ty));
        }
      }
      ...
      .progress-play-btn:hover {
        background: oklch(95% 0.002 265);
        border-color: oklch(70% 0.003 265);
      }
      ...
      .progress-bar-fill {
        height: 100%;
        border-radius: 100px;
        background: linear-gradient(90deg, oklch(62% 0.22 255), oklch(50% 0.2 255));
      ...
      .tier-pro {
        background: oklch(93% 0.07 255);
        color: oklch(40% 0.18 255);
      }
      .tier-business {
        background: oklch(93% 0.07 300);
        color: oklch(40% 0.18 300);
      }
      .tier-enterprise {
        background: oklch(93% 0.09 55);
        color: oklch(40% 0.18 55);
      }
      ...
      .locale-other-row:hover {
        background: oklch(96% 0.003 265);
      }
      ...
      .social-stat--heart:hover {
        color: oklch(55% 0.2 10);
      }
      .social-stat--heart.liked {
        color: oklch(55% 0.2 10);
      }
